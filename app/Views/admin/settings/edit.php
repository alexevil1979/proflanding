...
<form method="post" action="/admin/settings" class="form-grid">
    ...
    <label>Должность<input name="site_role" value="<?= e($s['site_role'] ?? '') ?>"></label>
    <label>Слоган<textarea name="site_tagline" rows="2"><?= e($s['site_tagline'] ?? '') ?></textarea></label>
    <label>Оффер (H1 fallback)<textarea name="hero_offer" rows="2"><?= e($s['hero_offer'] ?? '') ?></textarea></label>
    <label>Подзаголовок героя<textarea name="hero_sub" rows="3"><?= e($s['hero_sub'] ?? '') ?></textarea></label>
    ...
    <label>Город<input name="city" value="<?= e($s['city'] ?? '') ?>"></label>
    ...
    <label>FAQ JSON<textarea name="faq_json" rows="10"><?= e($s['faq_json'] ?? '') ?></textarea></label>
